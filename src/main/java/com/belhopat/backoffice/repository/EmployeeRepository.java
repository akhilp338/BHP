...
public interface EmployeeRepository extends JpaRepository<Employee, Long>, DataTablesRepository<Employee, Long> {
	...
	@Query("select e.employeeUser.id,CONCAT(e.employeeMaster.firstName,' ( ' ,e.employeeMaster.officialEmail, ' )') as data "
			+ "from Employee e where e.employeeUser is not null")

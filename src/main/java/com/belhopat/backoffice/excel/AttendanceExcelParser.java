...
public class AttendanceExcelParser extends BaseExcelParser {
	...
	public UploadResponse getParsedData(int days) {
		UploadResponse response = getErrorResponse();
		User loggedInUser = SessionManager.getCurrentUserAsEntity();
		List<String> fundingSources = getFundingSources();
		boolean containsDuplicates = containsDuplicates(fundingSources);
		...
		boolean isNoOfDaysFieldValid = isNoOfDaysFieldValid(days);
		if (!isNoOfDaysFieldValid) {
			response.setStatusMessage("asd");
			return response;
		}
		List<Employee> employees = new ArrayList<>();
		Map<String, Employee> employeeMap = new HashMap<>();
		getEmployeesMap(employees);
		List<Attendance> attendances = attendanceRepository.findAll();
//		response = getLiquiditySourcesLimits(days, liquiditySourcesLimits);
		return response;
	}

	private void getEmployeesMap(List<Employee> employees) {
...
//	}
//
//	private UploadResponse setDayNumbers(Row row, LiquiditySourcesLimitViewEntity lsLimit, int days) {
//		UploadResponse response = getErrorResponse();
